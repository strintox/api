...
async def start(update: Update, context: CallbackContext):
    ...
    welcome_message = (
        ...
        "Я - ваш персональный ассистент на базе Claude 3.7 Sonnet от Anthropic. "
        "Я могу отвечать на вопросы, анализировать фотографии, помогать с текстами и многое другое.\n\n"
        "📱 *Основные функции:*\n"
        "• Ответы на любые вопросы\n"
        "• Анализ изображений\n"
        "• Помощь в написании текстов\n"
        "• Работа с файлами (PDF, DOCX, TXT)\n"
    ...
    welcome_message += (
        ...
        "Просто начните общение, отправив сообщение, фотографию или файл! 🚀"
    )
...
async def help_command(update: Update, context: CallbackContext):
    help_text = (
        ...
        "• Отправляйте файлы (PDF, DOCX, TXT) для анализа\n"
...
def get_file_type(file_bytes):
    try:
        ...
        # Если и magic не сработал, определяем по сигнатуре файла вручную
        if file_bytes[:4] == b'%PDF':
            return 'application/pdf'
        elif file_bytes[:2] == b'PK':
            # DOCX и другие Office-файлы используют формат ZIP (начинаются с 'PK')
            # Здесь предполагаем, что это DOCX, но могут быть и другие типы Office-файлов
            return 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'
...
async def handle_document(update: Update, context: CallbackContext):
    ...
    try:
        ...
        try:
            ...
            if not file_type:
                await status_message.edit_text(f"❌ Не удалось определить тип файла. Пожалуйста, убедитесь, что файл имеет расширение .pdf, .docx или .txt")
                processing_files.remove(file_key)
                return
        
        if file_type not in SUPPORTED_FILE_TYPES:
            await status_message.edit_text(
                f"❌ Неподдерживаемый тип файла ({file_type}). Поддерживаемые форматы: PDF, DOCX, TXT."
            )
            processing_files.remove(file_key)
            return
        
        # Обновляем статус
        await status_message.edit_text(f"🔍 Извлекаю текст из {SUPPORTED_FILE_TYPES[file_type]} файла '{file_name}'...")
        ...
        query = update.message.caption or f"Проанализируй этот {SUPPORTED_FILE_TYPES[file_type]} файл: {file_name}"
        
        # Сообщаем пользователю
        await status_message.edit_text(f"💭 Отправляю запрос к Claude с текстом из файла ({len(limited_text)} символов)...")
...
def main():
    ...
    # Удаление неподдерживаемых типов сообщений (кроме файлов)
